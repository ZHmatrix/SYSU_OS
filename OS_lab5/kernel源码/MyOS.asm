...
;* 操作系统内核 8086汇编                           
;**************************************************
extrn  _cmain:near         ; 声明c主函数cmain()
extrn _print:near          ; 声明c程序函数print()
extrn  _HEX2DEC:near       ; 声明c程序函数HEX2DEC()
extrn  _BIN2DEC:near       ; 声明c程序函数BIN2DEC()
extrn _upper:near		   ; 声明c程序函数upper()
extrn _lower:near		   ; 声明c程序函数lower()
...
start:
	xor ax,ax				        		        ; AX = 0
	mov es,ax					                    ; ES = 0
	mov word ptr es:[33*4],offset My21h		        ; 设置My21h中断向量的偏移地址
	mov word ptr es:[33*4+2],cs				        ; 设置My21h中断向量的段地址=CS
